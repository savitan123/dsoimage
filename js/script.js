...
function initDSOImage() {
  ...
  const translations = {
    "en": {
      ...
      "plan_title": "Planned Session",
      "plan_subtitle": "Find targets and plan your imaging night.",
      "plan_mode": "Mode:",
      "plan_now": "Right Now",
      "plan_session": "Plan Session",
      "plan_from": "From Date:",
      "plan_to": "To Date:",
      "plan_start": "Start Time:",
      "plan_end": "End Time:",

      "opt_horizon": "10° (Horizon)",
      "opt_20deg": "20°",
      "opt_30deg": "30° (Good)",
      "opt_45deg": "45° (High)",
      "opt_60deg": "60° (Zenith)",
      "opt_highest_alt": "Highest Altitude",
      "opt_brightest": "Brightest (Mag)",
      "opt_all": "Everything",
      "opt_galaxies": "Galaxies",
      "opt_nebulae": "Nebulae",
      "opt_clusters": "Clusters",
      "opt_planetary": "Planetary Nebs",

      "cal_prev": "❮ Prev",
      "cal_next": "Next ❯",
      "cal_sun": "Sun", "cal_mon": "Mon", "cal_tue": "Tue", "cal_wed": "Wed",
      "cal_thu": "Thu", "cal_fri": "Fri", "cal_sat": "Sat",
      "cal_new": "🌑 New Moon (Dark Sky)",
      "cal_full": "🌕 Full Moon",
      "cal_meteor": "🌠 Meteor Shower Peak",
      "cal_best": "🔭 Best Target",
      "cal_imaging": "📷 Imaging Targets",
      "cal_observing": "🔭 Observing Targets"
    },
    "he": {
      ...
      "plan_title": "תכנון סשן",
      "plan_subtitle": "מצא מטרות ותכנן את לילת הצילום שלך.",
      "plan_mode": "מצב:",
      "plan_now": "עכשיו",
      "plan_session": "תכנן סשן",
      "plan_from": "מתאריך:",
      "plan_to": "עד תאריך:",
      "plan_start": "שעת התחלה:",
      "plan_end": "שעת סיום:",
      ...
      "opt_45deg": "45° (גבוה)",
      "opt_60deg": "60° (זנית)",
      "opt_highest_alt": "גובה מקסימלי",
      "opt_brightest": "הבהיר ביותר (כוכביות)",
      "opt_all": "הכל",
      "opt_galaxies": "גלקסיות",
      "opt_nebulae": "ערפיליות",
      "opt_clusters": "צבירים",
      "opt_planetary": "ערפיליות פלנטריות",

      "cal_prev": "❮ הקודם",
      "cal_next": "הבא ❯",
      "cal_sun": "ראשון", "cal_mon": "שני", "cal_tue": "שלישי", "cal_wed": "רביעי",
      "cal_thu": "חמישי", "cal_fri": "שישי", "cal_sat": "שבת",
      "cal_new": "🌑 ירח חדש (שמיים חשוכים)",
      "cal_full": "🌕 ירח מלא",
      "cal_meteor": "🌠 שיא מטאורים",
      "cal_best": "🔭 מטרה מומלצת",
      "cal_imaging": "📷 מטרות לצילום",
      "cal_observing": "🔭 מטרות לתצפית"
    }
  };
...
function buildMailtoHref() {
  ...
  return 'mailto:?subject=' + encodeURIComponent(subject + ' - ' + date) + '&body=' + encodeURIComponent(buildEmailBody());
}
...
let _glossaryMasterList = null;

function buildGlossaryMasterList() {
  if (_glossaryMasterList) return _glossaryMasterList;
  let masterList = [];
  ...
  _glossaryMasterList = masterList;
  return masterList;
}
...
function renderGlossary() {
  ...
  const isHe = (window.currentLang || 'en') === 'he';
  const masterList = buildGlossaryMasterList();
  ...
  items.forEach(item => {
    ...
    // Use Hebrew fields if available, fall back to English
    const displayTitle    = (isHe && item.title_he)    ? item.title_he    : item.title;
    const displayContent  = (isHe && item.content_he)  ? item.content_he  : item.content;
    const displayCategory = (isHe && item.category_he) ? item.category_he : item.category;
    ...
    row.innerHTML = `
      <div class="glossary-header">
        <div class="glossary-title-wrapper">
          <h4 class="glossary-title">${displayTitle}</h4>
          <span class="${tagClass}">${displayCategory}</span>
        </div>
    ...
    header.addEventListener('click', () => {
      ...
      document.querySelectorAll('.glossary-content').forEach(c => { c.style.maxHeight = null; });
  ...
  alphabet.forEach(letter => {
    ...
    a.innerText = letter;
    if (!activeLetters.has(letter)) a.classList.add("disabled");
    li.appendChild(a);
    navContainer.appendChild(li);
  });

  // Smooth scroll for A-Z nav
  navContainer.addEventListener('click', (e) => {
    if (e.target.tagName === 'A' && !e.target.classList.contains('disabled')) {
      e.preventDefault();
      const targetEl = document.getElementById(e.target.getAttribute('href').substring(1));
      if (targetEl) window.scrollTo({ top: targetEl.offsetTop - 120, behavior: 'smooth' });
    }
  });
...
function initGlossary() {
  ...
  window.addEventListener('languageChanged', () => renderGlossary());
  ...
  if (searchInput) {
    searchInput.addEventListener('input', (e) => {
      ...
      document.querySelectorAll('.glossary-letter-group').forEach(group => {
        const hasVisible = Array.from(group.querySelectorAll('.glossary-item')).some(i => i.style.display !== 'none');
